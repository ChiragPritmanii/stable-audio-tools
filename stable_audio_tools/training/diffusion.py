...
class DiffusionUncondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(self, model: DiffusionModelWrapper, lr: float = 1e-4):
        super().__init__()
...
class DiffusionCondTrainingWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast():
            ...
            v = self.diffusion(
                noised_inputs, t, cond=conditioning, cfg_dropout_prob=0.1, **extra_args
            )
            p.tick("diffusion")
    ...
    def on_train_batch_end(
        ...
        try:
            ...
            if random_samples:
                ...
                files_seconds_total = [
                    str(int(float(mediainfo(f)["duration"]))) for f in random_files
                ]
                files_seconds_start = [str(32)] * 4
                demo_data = list(
                    zip(random_files, files_seconds_start, files_seconds_total)
                )
            else:
                demo_data = [
                    [
                        demo_cond[0]["prompt"]["path"][0],
                        str(demo_cond[0]["seconds_start"]),
                        str(demo_cond[0]["seconds_total"]),
                    ],
                    [
                        demo_cond[1]["prompt"]["path"][0],
                        str(demo_cond[1]["seconds_start"]),
                        str(demo_cond[1]["seconds_total"]),
                    ],
                    [
                        demo_cond[2]["prompt"]["path"][0],
                        str(demo_cond[2]["seconds_start"]),
                        str(demo_cond[2]["seconds_total"]),
                    ],
                    [
                        demo_cond[3]["prompt"]["path"][0],
                        str(demo_cond[3]["seconds_start"]),
                        str(demo_cond[3]["seconds_total"]),
                    ],
                ]

            log_dict["demo_inputs"] = wandb.Table(
                columns=["prompt", "seconds_start", "seconds_total"], data=demo_data
            )
            ...
            if self.display_audio_cond:
                ...
                audio_inputs = (
                    audio_inputs.to(torch.float32).mul(32767).to(torch.int16).cpu()
                )
                ...
                log_dict[f"demo_audio_cond_melspec_left"] = wandb.Image(
            ...
            for cfg_scale in self.demo_cfg_scales:
                ...
                log_dict[f"demo_melspec_left_cfg_{cfg_scale}"] = wandb.Image(
...
class DiffusionCondInpaintTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        self.loss_modules = [MSELoss("v", "targets", weight=1.0, name="mse_loss")]
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast():
            p.tick("amp")
            v = self.diffusion(
                noised_inputs, t, cond=conditioning, cfg_dropout_prob=0.1
            )
            p.tick("diffusion")

            loss_info = {"v": v, "targets": targets}
        ...
        if self.diffusion.pretransform is not None:
            with torch.no_grad():
                ...
                if self.prior_type in [
                    PriorType.MonoToStereo,
                    PriorType.SourceSeparation,
                ]:
